...
// Ordered dictionary from longer phrases to single words to prevent partial substring collisions
const replacements = [
...
files.forEach(file => {
  ...
  // Perform terminology replacements
  replacements.forEach(([regex, replacement]) => {
    content = content.replace(regex, replacement);
  });

  fs.writeFileSync(filePath, content, 'utf8');
  console.log(`Cleaned terminology in ${file}`);
});

console.log('All module files successfully cleaned!');
